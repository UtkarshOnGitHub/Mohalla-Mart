import { Avatar, Box, Table, TableCaption, TableContainer, Tbody, Td, Text, Tfoot, Th, Thead, Tr, VStack } from '@chakra-ui/react'
  ...
  return (
    <TableContainer bgColor='rgba(0, 0, 0, 0)' color='white' w='100%' >
        <Table variant='simple' size='lg' >
            <Thead>
            <Tr >
                <Th color='white' w='5%'   textAlign='start' >No.</Th>
...

            {
                data?.map((item:any, j:any) => (
                    <Tr  >
                        <Td textAlign='start'>{j+1}</Td>
                        <Td textAlign='center' justifyContent='space-around' alignItems='center' gap={3} display='flex'>
                            <Box w='50%'>
                                <Avatar src={item.src || item.logo || 'https://bit.ly/dan-abramov'} borderRadius='2xl'/> 
                            </Box>
                            <VStack spacing={0} w='50%'>
                                <Text style={{whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}} fontSize='20px'>{item?.firstName}{" "}{item?.lastName} {item?.name}{item?.title?.slice(0,16)}</Text>
